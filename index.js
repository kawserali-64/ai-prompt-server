...
app.post("/api/review/add", async (req, res) => {
    ...
    try {
        const { promptId, rating, comment, userId, name, email } = req.body;

        // 1. validation
        if (!promptId || !rating || !userId) {
            return res.status(400).json({
                success: false,
                message: "promptId, rating, userId required",
            });
        }
        ...
        // 4. fetch all reviews (same prompt)
